...
serve(async (req) => {
  try {
    const { guild_id, style = 'rupture_frame', intensity = 'low', message = 'O padrão observa sem pressa.' } = await req.json()
    ...
    const { data } = await admin.from('inkosi_signals').insert({ guild_id, style, intensity, message, source: 'manual', created_by: caller.id, expires_at }).select().single()
    await admin.from('inkosi_trigger_log').insert({ guild_id, trigger_type: 'manual', style, payload: { intensity } })
    await audit(admin, { guild_id, actor_id: caller.id, action: 'inkosi_emit_manual', target_type: 'inkosi_signal', target_id: data?.id })
